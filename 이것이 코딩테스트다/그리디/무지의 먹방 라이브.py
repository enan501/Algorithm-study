...
def solution(food_times: list, k: int):
    ...
    food_times_indexed = sorted(enumerate(food_times), key=lambda q: q[1])  # 원래 인덱스와 함께 정렬
    ...
    for order, food_time in enumerate(food_times_indexed):
        ...
        if eat_time > left_time:  # 다 먹을 수 있는 음식이 없으면
            left_food_times = sorted(food_times_indexed[order:], key=lambda q: q[0])  # 남은 음식들 원래 인덱스 순으로 정렬
